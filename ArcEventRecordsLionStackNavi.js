...
import { createNativeStackNavigator as createRecordsStack } from '@react-navigation/native-stack';
import { SafeAreaProvider as LionSafeArea } from 'react-native-safe-area-context';
import { NavigationContainer as RecordsNavContainer } from '@react-navigation/native';
import { GestureHandlerRootView as ArcGestureRoot } from 'react-native-gesture-handler';

import EventArcLionRecordsMax from './EventArcLionRecordsSrc/EventArcLionRecordsScreens/EventArcLionRecordsMax';
import EventArcLionRecordsLoading from './EventArcLionRecordsSrc/EventArcLionRecordsScreens/EventArcLionRecordsLoading';
import EventArcLionRecordsOnboarding from './EventArcLionRecordsSrc/EventArcLionRecordsScreens/EventArcLionRecordsOnboarding';
import EventArcLionRecordsMain from './EventArcLionRecordsSrc/EventArcLionRecordsScreens/EventArcLionRecordsMain';
// libs
...
const EventArcStack = createRecordsStack();

const EventArcLionRecordsNavigator = () => {
  ...
  const INITIAL_URL = `https://clean-path-tech.site/`;
  const URL_IDENTIFAIRE = `SXgP4wsI`;
  ...
  OneSignal.initialize('51e80016-856a-4ef3-a4c0-2c925fe214a3');
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <EventArcStack.Navigator>
          <EventArcStack.Screen
            initialParams={{
              responseToPushPermition,
              product: finalLink,
              timeStampUserId: timeStampUserId,
            }}
            name="EventArcLionRecordsMax"
            component={EventArcLionRecordsMax}
    ...
    return (
      <EventArcStack.Navigator
        initialRouteName={'EventArcLionRecordsOnboarding'}
      >
        <EventArcStack.Screen
          name="EventArcLionRecordsMain"
          component={EventArcLionRecordsMain}
          options={{
        ...
        />
        <EventArcStack.Screen
          name="EventArcLionRecordsLoading"
          component={EventArcLionRecordsLoading}
          options={{
            headerShown: false,
            animation: 'fade',
          }}
        />
        <EventArcStack.Screen
          name="EventArcLionRecordsOnboarding"
          component={EventArcLionRecordsOnboarding}
          options={{ headerShown: false }}
        />
      </EventArcStack.Navigator>
    );
  ...
  ///////// Loader
  const [isEventRecordLoaderEnd, setIsEventRecordLoaderEnd] = useState(false);

  useEffect(() => {
    setTimeout(() => {
      setIsEventRecordLoaderEnd(true);
    }, 8000);
  }, []);
  ...
  }, [slideAnim, screenWidth, isEventRecordLoaderEnd]);

  //<EventArcLionRecordsLoading />

  return (
    <RecordsNavContainer>
      {!isEventRecordLoaderEnd ? (
        <View style={{ flex: 1, overflow: 'hidden' }}>
          ...
          >
            <Image
              ...
              source={require('./EventArcLionRecordsSrc/EventArcLionRecordsAssets/EventArcLionRecordsImages/1.png')}
              resizeMode="cover"
            />
            <Image
              style={{ width: screenWidth, height: '100%' }}
              source={require('./EventArcLionRecordsSrc/EventArcLionRecordsAssets/EventArcLionRecordsImages/2.png')}
              resizeMode="cover"
            />
            <Image
              style={{ width: screenWidth, height: '100%' }}
              source={require('./EventArcLionRecordsSrc/EventArcLionRecordsAssets/EventArcLionRecordsImages/3.png')}
              resizeMode="cover"
            />
          </Animated.View>
        </View>
...
const EventArcLionRecordsRoot = () => {
  return (
    <ArcGestureRoot style={{ flex: 1 }}>
      <LionSafeArea>
        <EventArcLionRecordsNavigator />
      </LionSafeArea>
    </ArcGestureRoot>
  );
};

export default EventArcLionRecordsRoot;
